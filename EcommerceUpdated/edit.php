...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $category = $_POST['category']; // Capture the updated category
    ...
    if (!empty($image)) {
        ...
        $updateStmt = $con->prepare("UPDATE products SET name = ?, price = ?, description = ?, image = ?, category = ? WHERE id = ?");
        $updateStmt->execute([$name, $price, $description, $image_path, $category, $product_id]);
    } else {
        $updateStmt = $con->prepare("UPDATE products SET name = ?, price = ?, description = ?, category = ? WHERE id = ?");
        $updateStmt->execute([$name, $price, $description, $category, $product_id]);
    }
    header('Location: admin.php');
    exit();
}
